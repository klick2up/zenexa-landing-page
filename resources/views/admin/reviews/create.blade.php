...
@section('title', 'Add Testimonial / Review | Klick2Up')
...
<div class="space-y-8 animate-fade-in-up">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-display font-bold text-primary">Add Testimonial</h2>
            <p class="text-xs text-gray-400 mt-1">Create a new client review to display in the homepage marquee</p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-200/80 shadow-sm p-8 w-full">
        <form id="review-form" action="{{ route('admin.reviews.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="client_name" id="client_name" value="{{ old('client_name') }}" required class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all placeholder-gray-400" placeholder="e.g. Marcus Chen">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="client_title" id="client_title" value="{{ old('client_title') }}" required class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all placeholder-gray-400" placeholder="e.g. CTO, Vertex Logistics">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="project_name" id="project_name" value="{{ old('project_name') }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all placeholder-gray-400" placeholder="e.g. HealthConnect Pro">
                </div>
                ...
                {{-- Client Avatar --}}
                <div class="space-y-2">
                    <label for="client_avatar" class="block text-xs font-bold uppercase tracking-wider text-gray-400">Client Avatar Image</label>
            ...
            <div class="space-y-2">
                ...
                <textarea name="review_text" id="review_text" rows="5" required class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all placeholder-gray-400" placeholder="Type the review text provided by the client...">{{ old('review_text') }}</textarea>
            </div>
