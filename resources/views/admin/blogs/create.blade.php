...
@section('title', 'Write New Blog Post | Klick2Up')
...
<div class="space-y-8 animate-fade-in-up">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-display font-bold text-primary">Write New Article</h2>
            <p class="text-xs text-gray-400 mt-1">Compose and publish a new technical article on the public site</p>
        </div>
        ...
        <form id="blog-form" action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all placeholder-gray-400" placeholder="e.g. The Future of PHP Architectures">
                </div>

                {{-- Slug --}}
                <div class="space-y-2">
                    <label for="slug" class="block text-xs font-bold uppercase tracking-wider text-gray-400">Custom URL Slug (Optional)</label>
                    <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all placeholder-gray-400" placeholder="e.g. future-of-php-architectures">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select name="badge" id="badge" required class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all">
                        <option value="Laravel" {{ old('badge') == 'Laravel' ? 'selected' : '' }}>Laravel</option>
                        <option value="ReactJS" {{ old('badge') == 'ReactJS' ? 'selected' : '' }}>ReactJS</option>
                        <option value="NodeJS" {{ old('badge') == 'NodeJS' ? 'selected' : '' }}>NodeJS</option>
                        <option value="ERP" {{ old('badge') == 'ERP' ? 'selected' : '' }}>ERP</option>
                        <option value="AI" {{ old('badge') == 'AI' ? 'selected' : '' }}>AI</option>
                        <option value="Design" {{ old('badge') == 'Design' ? 'selected' : '' }}>Design</option>
                        <option value="Growth" {{ old('badge') == 'Growth' ? 'selected' : '' }}>Growth</option>
                    </select>
                </div>

                {{-- Image Cover --}}
                <div class="space-y-2">
                    <label for="image" class="block text-xs font-bold uppercase tracking-wider text-gray-400">Cover Image</label>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="author_name" id="author_name" value="{{ old('author_name', Auth::user()->name) }}" required class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all placeholder-gray-400" placeholder="e.g. Hardik Patel">
                </div>

                {{-- Author Title --}}
                <div class="space-y-2">
                    <label for="author_title" class="block text-xs font-bold uppercase tracking-wider text-gray-400">Author Title</label>
                    <input type="text" name="author_title" id="author_title" value="{{ old('author_title') }}" required class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all placeholder-gray-400" placeholder="e.g. Lead PHP Architect">
                </div>

                {{-- Author Avatar --}}
                <div class="space-y-2 md:col-span-2">
                    <label for="author_avatar" class="block text-xs font-bold uppercase tracking-wider text-gray-400">Author Avatar Image</label>
            ...
            <div class="space-y-2">
                ...
                <textarea name="summary" id="summary" rows="3" required class="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all placeholder-gray-400" placeholder="A single paragraph summarizing the post for cards...">{{ old('summary') }}</textarea>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="hidden" name="body_content" value="{{ old('body_content', '') }}" id="body_content">
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" class="bg-accent hover:bg-rose-700 text-white px-8 py-3 rounded-full font-bold text-xs transition duration-300 shadow-md hover:shadow-lg flex items-center gap-2">
                    <i data-lucide="check" class="w-4 h-4"></i> Publish Article
                </button>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Set previous content if validation error occurred
        @if(old('body_content'))
            quill.root.innerHTML = {!! json_encode(old('body_content')) !!};
        @endif
